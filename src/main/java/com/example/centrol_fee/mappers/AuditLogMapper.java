...
public interface AuditLogMapper {
    ...
    @Insert("INSERT INTO audit_logs(user_id, username, action, entity_name, entity_id, description, old_value, new_value, ip_address, created_at) " +
            "VALUES(#{userId}, #{username}, #{action}, #{entityName}, #{entityId}, #{description}, #{oldValue}, #{newValue}, #{ipAddress}, NOW())")
    ...
    @Select("SELECT id, user_id AS userId, username, action, entity_name AS entityName, entity_id AS entityId, " +
            "description, old_value AS oldValue, new_value AS newValue, ip_address AS ipAddress, created_at AS createdAt " +
